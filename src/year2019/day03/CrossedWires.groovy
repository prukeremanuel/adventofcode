...
//    static String inputWireB = "U62,R66,U55,R34,D71,R55,D58,R83"
    ...
    static navigate(List<String> input) {
        input.each {
            switch (it[0]) {
                ...
                case "R":
                    for (int i = 1; i <= it[1..-1].toInteger(); i++) {
                        routeA.add([routeA.last()[0], routeA.last()[1] + 1])
                    }
                    break
    ...
    static navigateSecond(List<String> input, List<List<Integer>> route) {
        input.each {
            switch (it[0]) {
                case "U":
                    for (int i = 1; i <= it[1..-1].toInteger(); i++) {
                        if (route.contains([routeB.last()[0] + 1, routeB.last()[1]])) {
                            if(routeA.indexOf([routeB.last()[0] + 1, routeB.last()[1]]) +
                                    routeB.size() < steps) {
                                steps = routeA.indexOf([routeB.last()[0] + 1, routeB.last()[1]]) +
                                        routeB.size()
                            }
                            routeB.add([routeB.last()[0] + 1, routeB.last()[1]])
                        } else {
                            routeB.add([routeB.last()[0] + 1, routeB.last()[1]])
                        }
                    }
                ...
                case "R":
                    for (int i = 1; i <= it[1..-1].toInteger(); i++) {
                        if (route.contains([routeB.last()[0], routeB.last()[1] + 1])) {
                            if(routeA.indexOf([routeB.last()[0], routeB.last()[1] + 1]) +
                                    routeB.size() < steps) {
                                steps = routeA.indexOf([routeB.last()[0], routeB.last()[1] + 1]) +
                                        routeB.size()
                            }
                            routeB.add([routeB.last()[0], routeB.last()[1] + 1])
                        }
                        else {
                            routeB.add([routeB.last()[0], routeB.last()[1] + 1])
                        }
...
//                    for (int i = 1; i <= it[1..-1].toInteger(); i++) {
//                        if (route.contains([routeB.last()[0] + 1, routeB.last()[1]])) {
//                            if((Math.abs(routeB.last()[0] + 1) + Math.abs(routeB.last()[1]))<distance) {
//                                distance = Math.abs(routeB.last()[0] + 1) + Math.abs(routeB.last()[1])
//                            }
//                            routeB.add([routeB.last()[0] + 1, routeB.last()[1]])
//                        } else {
//                            routeB.add([routeB.last()[0] + 1, routeB.last()[1]])
...
//                    for (int i = 1; i <= it[1..-1].toInteger(); i++) {
//                        if (route.contains([routeB.last()[0], routeB.last()[1] + 1])) {
//                            if((Math.abs(routeB.last()[0]) + Math.abs(routeB.last()[1] + 1))<distance) {
//                                distance = Math.abs(routeB.last()[0]) + Math.abs(routeB.last()[1] + 1)
//                            }
//                            routeB.add([routeB.last()[0], routeB.last()[1] + 1])
//                        }
//                        else {
...
//    }
    ...
    public static void main(String[] args) {
        inputWireAAsList = inputAsList(inputWireA)
        inputWireBAsList = inputAsList(inputWireB)
        println inputWireAAsList
        println inputWireBAsList

        navigate(inputWireAAsList)
        navigateSecond(inputWireBAsList, routeA)
//        navigate(["R75","D30","R83","U83","L12","D49","R71","U7","L72"])
//        navigateSecond(["U62","R66","U55","R34","D71","R55","D58","R83"], routeA)
//        println routeA
//        println routeB
        println steps
